...
std::vector<double> hQ2_range{200,0.1,10},hx_range{200,0.01,1.2},hW_range{200,0.1,5},hz_range{200,0.01,1.2},hMMpi_range{200,-0.5,8},hpmiss_range{200,-10,10},hemiss_range{200,-10,10},htarg_range{200,-10,10};
// ---
...
// Main function
int get_good_pionct_ev(int rnum,                 // Run number to analyze
		     ...
		     std::string outdirplot="HISTOGRAMS/COIN/PDF", // Path to directory to save output plots
		       std::string outfilebase="output_get_good_pionct_ev", // output filename prefix
		       std::string referencesimfile="") // reference file for the simulations
{
  ...
  // Missing mass= |e+A-e'-pi|=-Q2+m_A^2+m_pi^2 + 2(m_A-E_pi)*nu+2*q dot vec pi-2*E_pi m_A
  ...
  if(referencesimfile==""){
    ...
    //try to autodetect which file to use based on the MA value
    std::string targ=MA<1? "H": (MA<2 ? "D" : "C");
    int filenum= Q2mean< 5.7 ? 4 : (Q2mean < 7 ? 3 : (Q2mean < 8 ? 2 : 1));
    std::cout << "Q2mean " << Q2mean << " filenum " << filenum << std::endl;
    referencesimfile=Form("simc/%s_rp%d.root", targ.c_str(), filenum);
  }

  ROOT::RDataFrame sim_rdf("h10",referencesimfile.c_str()); 
  ...
  auto sim_rdf_raw = sim_rdf.Define("x","Q2/(2*0.938*nu)").Define("z", "sqrt(phad*phad+.1396*.1396)/nu");
  ...
  TH1F *hemiss = (TH1F*)data_rdf_raw.Filter(anacuts)
    .Histo1D({"hemiss","",int(hemiss_range[0]),hemiss_range[1],hemiss_range[2]}, "P.kin.secondary.emiss_nuc")->Clone();
  TH1F *hpmiss_x = (TH1F*)data_rdf_raw.Filter(anacuts)
    .Histo1D({"hpmiss_x","",int(hpmiss_range[0]),hpmiss_range[1],hpmiss_range[2]}, "P.kin.secondary.pmiss_x")->Clone();
  TH1F *hpmiss_y = (TH1F*)data_rdf_raw.Filter(anacuts)
    .Histo1D({"hpmiss_y","",int(hpmiss_range[0]),hpmiss_range[1],hpmiss_range[2]}, "P.kin.secondary.pmiss_y")->Clone();
  TH1F *hpmiss_z = (TH1F*)data_rdf_raw.Filter(anacuts)
    .Histo1D({"hpmiss_z","",int(hpmiss_range[0]),hpmiss_range[1],hpmiss_range[2]}, "P.kin.secondary.pmiss_z")->Clone();

  /*TH1F *htarg_x = (TH1F*)data_rdf_raw.Filter(anacuts)
    .Histo1D({"htarg_x","",int(htarg_range[0]),htarg_range[1],htarg_range[2]}, "P.react.x")->Clone();
  TH1F *htarg_y = (TH1F*)data_rdf_raw.Filter(anacuts)
  .Histo1D({"htarg_y","",int(htarg_range[0]),htarg_range[1],htarg_range[2]}, "P.react.y")->Clone();*/
  TH1F *htarg_z = (TH1F*)data_rdf_raw.Filter(anacuts)
    .Histo1D({"htarg_z","",int(htarg_range[0]),htarg_range[1],htarg_range[2]}, "P.react.z")->Clone();

  
  
  // simulation plots
  TH1F *hxsim = (TH1F*)sim_rdf_raw.Histo1D({"hxsim","",int(hx_range[0]),hx_range[1], hx_range[2]}, "x", "Weight")->Clone();
  TH1F *hzsim = (TH1F*)sim_rdf_raw.Histo1D({"hzsim","",int(hz_range[0]),hz_range[1], hz_range[2]}, "z", "Weight")->Clone();
  TH1F *hQ2sim = (TH1F*)sim_rdf_raw.Histo1D({"hQ2sim","",int(hQ2_range[0]),hQ2_range[1], hQ2_range[2]}, "Q2", "Weight")->Clone();
  TH1F *hWsim = (TH1F*)sim_rdf_raw.Histo1D({"hWsim","",int(hW_range[0]),hW_range[1], hW_range[2]}, "W", "Weight")->Clone(); 
  TH1F *hMMsim = (TH1F*)sim_rdf_raw.Histo1D({"hMMsim","",int(MM_range[0]),MM_range[1], MM_range[2]}, "mmnuc", "Weight")->Clone();  
  
  
  hxsim->SetLineColor(kRed);
  hxsim->Scale(hx->Integral()/hxsim->Integral());
  hzsim->SetLineColor(kRed);
  hzsim->Scale(hz->Integral()/hzsim->Integral());
  hQ2sim->SetLineColor(kRed);
  hQ2sim->Scale(hQ2->Integral()/hQ2sim->Integral());
  hWsim->SetLineColor(kRed);
  hWsim->Scale(hW->Integral()/hWsim->Integral());  
  hMMsim->SetLineColor(kRed);
  hMMsim->Scale(hMM->Integral()/hMMsim->Integral());  
  ...
  hxsim->Write("", TObject::kOverwrite);
  //
  cphys->cd(2);
  hQ2->Draw();
  hQ2sim->Draw("SAME");
  hQ2->Write("",TObject::kOverwrite);
  hQ2sim->Write("",TObject::kOverwrite);
  //
  cphys->cd(3);
  hz->Draw();
  hzsim->Draw("SAME");
  hz->Write("",TObject::kOverwrite);
  hzsim->Write("",TObject::kOverwrite);
  //
  cphys->cd(4);
  hW->Draw();
  hWsim->Draw("SAME");
  hW->Write("",TObject::kOverwrite);
  hWsim->Write("",TObject::kOverwrite);
  //
  cphys->cd(5);
  hMMpi->Draw();
  ...
  TCanvas *cpmiss = new TCanvas("cpmiss","cpmiss",1500,1500);
  cpmiss->Divide(2,2);
  gStyle->SetOptStat(1111);
  //
  cpmiss->cd(1);
  hpmiss_x->Draw();
  //hpmiss_xsim->Draw("SAME");
  hpmiss_x->Write("",TObject::kOverwrite);
  //hpmiss_xsim->Write("", TObject::kOverwrite); 
  //
  cpmiss->cd(2);
  hpmiss_y->Draw();
  //hpmiss_ysim->Draw("SAME");
  hpmiss_y->Write("",TObject::kOverwrite);
  //hpmiss_ysim->Write("", TObject::kOverwrite);
  //
  cpmiss->cd(3);
  hpmiss_z->Draw();
  //hpmiss_zsim->Draw("SAME");
  hpmiss_z->Write("",TObject::kOverwrite);
  //hpmiss_xsim->Write("", TObject::kOverwrite); 
  //
  cpmiss->cd(4);
  hemiss->Draw();
  //hemiss_sim->Draw("SAME");
  hemiss->Write("",TObject::kOverwrite);
  //hemiss_sim->Write("", TObject::kOverwrite);
  cpmiss->Write("",TObject::kOverwrite);

  TCanvas *ctarg = new TCanvas("ctarg","ctarg",500,500);
  //ctarg->Divide(3,1);
  gStyle->SetOptStat(1111);
  //
  /*ctarg->cd(1);
  htarg_x->Draw();
  //htarg_xsim->Draw("SAME");
  htarg_x->Write("",TObject::kOverwrite);
  //htarg_xsim->Write("", TObject::kOverwrite); 
  //
  ctarg->cd(2);
  htarg_y->Draw();
  //htarg_ysim->Draw("SAME");
  htarg_y->Write("",TObject::kOverwrite);
  //htarg_ysim->Write("", TObject::kOverwrite);
  //
  ctarg->cd(3);*/
  htarg_z->Draw();
  //htarg_zsim->Draw("SAME");
  htarg_z->Write("",TObject::kOverwrite);
  //htarg_xsim->Write("", TObject::kOverwrite); 
  ctarg->Write("",TObject::kOverwrite);
  ...
  // Plotting the missing mass
  TCanvas *cMM = new TCanvas("cMM","cMM",1500,800);
  hMM->Draw();
  hMMsim->Draw("SAME");
  hMM->Write("",TObject::kOverwrite);
  hMMsim->Write("",TObject::kOverwrite);
  cMM->Write("",TObject::kOverwrite);
  ...
  cMM->SaveAs(Form("%s",outplot.Data()));    
...
/* Plots cut region for given x range */
{
  ...
  cutRegion->Draw();*/
  TLine * line = new TLine(xmin, 0, xmin, UnfoldyNDC(0.9));
  line->SetLineColor(fillcolor);
  line->Draw();
  line = new TLine(xmax, 0, xmax, UnfoldyNDC(0.9));
  line->SetLineColor(fillcolor);
  line->Draw();
}
//----------------------------------------------------------
