void replay_production_coin_hElec_pProt_pdcRef(Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/coin_replay_production_pdcRef_%d_%d.root";
  
  // Load global parameters
  gHcParms->Define("gen_run_number", "Run Number", RunNumber);
  gHcParms->AddString("g_ctp_database_filename", "DBASE/COIN/standard_pdcRef.database");
  ...
  gHcDetectorMap->Load("MAPS/COIN/DETEC/coin_pdcRef.map");
  ...
  analyzer->PrintReport("TEMPLATES/COIN/PRODUCTION/coin_production.template",
  			Form("REPORT_OUTPUT/COIN/PRODUCTION/replay_coin_production_pdcRef_%d_%d.report", RunNumber, MaxEvent));  // optional
