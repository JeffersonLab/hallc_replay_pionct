void replay_production_coin_hElec_pPion (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* RunFileNamePattern = "pionct_production_%05d.dat.0";  
  ...
  Double_t MA = 0.938272; // Default fallback to proton mass
  if (gHcParms->FindObject("gtargmass_amu")) {
    // Extract and convert AMU to GeV if needed
    Double_t targ_amu = *(Double_t*)gHcParms->FindObject("gtargmass_amu");
    MA = targ_amu * 0.931494e-3; 
  } else {
    cout << "Warning: gtargmass_amu not found. Using default mass." << endl;
  }
  ...
  THcPrimaryKine* hkin_primary = new THcPrimaryKine("H.kin.primary", "HMS Single Arm Kinematics", "H", "H.rb", MA);
