         ...
         TString QVal = "4.3")  // Q2 value for simc plot (8.5, 7.5, 6.5, 5)
{
  ...
  double charge = ExtractValueFromReportFile(inrepfile, "SHMS BCM2  Beam Cut Charge", ':', 0); //mC
   ...
   {
      MA = data_rdf.Filter(anacuts).Mean("H.kin.primary.MA").GetValue(); 
   }
...
// ---- HGC geometry cut + efficiency calculation ----
  ...
  double hgcer_eff = (nMatched > 0) ? nFired / nMatched : 0.;
  double hgcer_eff_err = (nMatched > 0) ? sqrt(hgcer_eff*(1.-hgcer_eff)/nMatched) : 0.;

   std::cout << "------" << std::endl;
  std::cout << " SHMS HGC Efficiency (geom cut applied): " 
            << hgcer_eff << " +/- " << hgcer_eff_err 
            << "  (" << nFired << "/" << nMatched << ")" << std::endl;
  std::cout << "------" << std::endl;
